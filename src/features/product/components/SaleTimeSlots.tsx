  ...
  return (
    <nav className='pt-2' aria-label='Flash Sale Time Slots'>
      <div className='relative border-b border-(--color-border-1)'>
        ...
        <div className='flex w-full items-stretch h-full xl:pt-4 overflow-x-auto scrollbar-custom'>
          {timeSlots.map((tab) => {
            const isActive = tab.key === activeTab

            return (
              <div className='relative flex-1 min-w-[120px] xl:min-w-[160px]' key={tab.key}>
                <button
                  type='button'
                  onClick={() => handleTabChange(tab.key)}
                  className={`relative text-center cursor-pointer h-full w-full flex flex-col items-center justify-center xl:gap-2 whitespace-nowrap pb-2 xl:pb-4 transition-colors focus:outline-none ${isActive ? 'text-(--color-orange-1)' : 'text-(--color-gray-2)'
                    }`}
                ...
                >
                  ...
                  {tab.subtitle && (
                    <span
                      className={`text-[12px] xl:text-base leading-4 whitespace-nowrap ${isActive ? 'text-(--color-orange-1)' : 'text-(--color-gray-2)'
                        }`}
                    >
                      {tab.subtitle}
                    </span>
